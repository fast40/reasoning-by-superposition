...
with torch.profiler.profile(
    ...
    for pass_idx in range(n_latent_passes):
        print(f"{ts()} pass {pass_idx}/{n_latent_passes} - building tensor_list")
        tensor_list = [
            [inputs_embeds[batch_idx, pos, :] for pos in range(inputs_embeds.shape[1])]
            for batch_idx in range(inputs_embeds.shape[0])
        ]

        print(f"{ts()} pass {pass_idx}/{n_latent_passes} - filling indices")
        ...
        print(f"{ts()} pass {pass_idx}/{n_latent_passes} - torch.stack")

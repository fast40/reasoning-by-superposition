...
class Coconut(nn.Module):
    ...
    def __init__(
        ...
        self.skip_inject_layer = skip_inject_layer
        self.skip_extract_layer = skip_extract_layer

        self._is_gpt2 = isinstance(base_causallm, GPT2LMHeadModel)
    ...
    def layers(self):
        if self._is_gpt2:
            return self.base_causallm.transformer.h
    ...
    @property
    def wpe(self):
        """GPT2 learned positional embeddings."""
        return self.base_causallm.transformer.wpe

    # ------------------------------------------------------------------
    # GPT2 manual forward
    # ------------------------------------------------------------------

    def _forward_latent_pass_gpt2(
        self, original_embeds, continuous_thought,
        attention_mask, position_ids, past_key_values,
    ):
        """
        Manual forward for latent recurrence with configurable layer skip (GPT2).
        ...
        # GPT2 needs position embeddings added to inputs before layers
        pos_embeds = self.wpe(position_ids)
        original_embeds = original_embeds + pos_embeds
        continuous_thought = continuous_thought + pos_embeds

        # Build causal attention mask (additive: 0 for attend, -inf for mask)
        # attention_mask is (batch, total_seq_len) with 1s for valid positions
        causal_mask = attention_mask[:, None, None, :]
        causal_mask = causal_mask.to(dtype=continuous_thought.dtype)
        causal_mask = (1.0 - causal_mask) * torch.finfo(continuous_thought.dtype).min

        # Unpack past KV (list of (k, v) per layer, or None)
        if past_key_values:
            layer_pasts = list(past_key_values)
        else:
            layer_pasts = [None] * n
        ...
        kv_tuple = tuple(presents)

        return logits, new_continuous_thought, kv_tuple

    # ------------------------------------------------------------------
    # Llama manual forward
    # ------------------------------------------------------------------

    def _forward_latent_pass_llama(
        self, original_embeds, continuous_thought,
        attention_mask, position_ids, past_key_values,
    ):
        """
        Manual forward for latent recurrence with configurable layer skip (Llama).
    ...
    def _forward_latent_pass(self, *args, **kwargs):
        if self._is_gpt2:
            return self._forward_latent_pass_gpt2(*args, **kwargs)
        return self._forward_latent_pass_llama(*args, **kwargs)
